...
class MinHash(object):
    ...
    __slots__ = ('permutations', 'hashvalues', 'seed', 'hashobj', 'tipo')

    def __init__(self, num_perm=128, seed=1, hashobj=sha1,
            hashvalues=None, permutations=None, tipo='minhash'):
        '''
        ...
        if hashvalues is not None:
            ...
            if tipo == 'minhash':
                ...
                self.hashvalues = np.array([1 for _ in range(num_perm)], dtype=np.int)
            else:
                raise ValueError("O tipo "+tipo+" nao esta implementado")
    ...
    def update(self, b):
        ...
        phv = (a * hv + b) % _mersenne_prime
        #print phv
        #self.hashvalues = np.minimum(phv, self.hashvalues)
        self.hashvalues = np.maximum(phv, self.hashvalues)
